...
#
# From SGE/qsub to PBS/qsub,   Weibin Zhang
...
for ($x = 1; $x <= 3; $x++) { # This for-loop is to 
  ...
  while (@ARGV >= 2 && $ARGV[0] =~ m:^-:) {
    ...
    if ($switch eq "-V") {
      $qsub_opts .= "-V ";
      die "-V option is currently not allowed in PBS qsub!\n";
    } else {
      $option = shift @ARGV;
      if ($switch eq "-sync" && $option =~ m/^[yY]/) {
        $sync = 1;
        die "Synchronization model is not allowed in PBS qsub!\n";
      }
      ...
      if ($switch eq "-pe") { # e.g. -pe smp 5
        ...
        $qsub_opts .= "$option2 ";
        $nof_threads = $option2;
        die "-pe is currently not allowed in PBS qsub!\n";
      }
    }
  }
...
## PBS qsub
$qsub_cmd = "qsub -S /bin/bash -v PATH -d $cwd -j oe -o $queue_logfile $qsub_opts $queue_array_opt $queue_scriptfile >>$queue_logfile 2>&1";
print Q "# $qsub_cmd\n";
...
if (! $sync) { # We're not submitting with -sync y, so we
  ...
  $pbs_job_id=`grep "hnlgdx" $queue_logfile | awk '{ print \$1 }' | sed 's|\\\..*||'`;
  chomp($pbs_job_id);
  $check_sge_job_ctr=1;
  #
  $wait = 0.1;
    ...
    while (! -f $f) {
      ...
      if (($check_sge_job_ctr++ % 10) == 0) { # Don't run qstat too often, avoid stress on SGE.
        ...
        if ($ret>>8 == 1) { # Job does not seem to exist 
          ...
          print STDERR "queue.pl: Warning: qstat command returned status $ret (qstat -t $pbs_job_id,$!)\n";
          exit(1);
        }
      }
    }
